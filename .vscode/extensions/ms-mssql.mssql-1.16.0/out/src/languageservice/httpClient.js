...
class HttpClient {
     ...
     */
    downloadFile(urlString, pkg, logger, statusView, proxy, strictSSL, authorization) {
    ...
    getHttpClientOptions(url, proxy, strictSSL, authorization) {
        ...
        if (url.protocol === 'https:') {
            let httpsOptions = {
                ...
                path: url.path,
                agent: agent,
                rejectUnauthorized: proxy_1.isBoolean(strictSSL) ? strictSSL : true
            };
    ...
    handleSuccessfulResponse(pkg, response, logger, statusView) {
        return new Promise((resolve, reject) => {
            ...
            response.on('error', (err) => {
                reject(new interfaces_1.PackageError(`Response error: ${err.code || 'NONE'}`, pkg, err));
            });
